...
export abstract class FileUploadAbstract implements OnInit, OnDestroy {
  ...
  protected setEvents(): void {
    ...
    this.subscriptions.push(
      this.control.eventsChanges.subscribe((event: FileEvent) =>
        this.triggerEvent(event)
      )
    );

    this.subscriptions.push(
      this.control.acceptChanges.subscribe((accept: string) =>
        this.updateAcceptAttr(accept)
      )
    );

    this.subscriptions.push(
      this.control.multipleChanges.subscribe((isMultiple: boolean) =>
        this.toggleMultiple(isMultiple)
      )
    );
  }
  ...
  protected checkAndSetMultiple(): void {
    ...
    const isMultiple = !(
      this.isMultiple === false || (this.isMultiple as string) === "false"
    );
